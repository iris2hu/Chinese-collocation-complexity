...
def loadCollData(file):
    d = {}
    for line in open(file):
        coll, typ, freq, MI = line.strip().split('\t')
        k = coll.replace(' ', '\t') + '\t' + typ
        d[k] = [int(freq), float(MI)]
    print(file, ': loaded %s collocations' % len(d))
    return d


textbook_colls = loadCollData('./data/coll_textbook_index.txt')
wiki_colls = loadCollData('./data/coll_wiki_index.txt')
...
def isLowFreqColl(coll):
    tf, tm, wf, wm = 0, 0, 0, 0
    if coll in textbook_colls:
        tf, tm = textbook_colls[coll]
    if coll in wiki_colls:
        wf, wm = wiki_colls[coll]
    if (tf < 6 and wf < 30) and (tm > 0 or wm > 0):
        return True
    else:
        return False
